...
```{r}
df.out %>% 
  mutate(check.condition = (
    (with.any.response==1) | 
      (with.any.response==0 & record.status=="FRAGMENT RECORD")
    )
    ) %>%
...
The table shows that all EMAs in `df.out` have the timestamp `delivered.unixts`. However, the EMAs having `responded==Missing` and `is.missing.assessment.unixts==FALSE` are unexpected. 
