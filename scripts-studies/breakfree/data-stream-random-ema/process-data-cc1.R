...
for(i in 1:N){
  ...
  # Save changes
  list.df.raw.random.DATA.cc1[[i]] <- df.raw
...
source(file.path(path.breakfree.code, "data-stream-random-ema/grab-single-response-items-cc1.R"))
...
# Grab items with responses find.this.string
source(file.path(path.breakfree.code, "data-stream-random-ema/grab-multiple-response-items-cc1.R"))
df.resp.cc1 <- df.resp.cc1 %>% mutate_at(vars(grep(pattern = "item.", x = .)), as.character)
