...
for(i in 1:length(ids.cc1)){
  ...
  idx <- grepl(pattern = "EMA+RANDOM_EMA+PHONE+processed.csv", 
               x = all.files, 
               fixed = TRUE)
  ...
  idx2 <- grepl(pattern = "EMA+RANDOM_EMA+PHONE.csv.zip", 
                x = all.files, 
                fixed = TRUE)
  # Pick out corresponding files
...
list.df.raw.random.DATA.cc1 <- list.df.raw
remove(list.df.raw)

ids.backup.random.cc1 <- unlist(list.ids.backup)
ids.none.random.cc1 <- unlist(list.ids.none)

# -----------------------------------------------------------------------------
# Save objects into an RData file
# -----------------------------------------------------------------------------
save(list.df.raw.random.DATA.cc1, ids.backup.random.cc1, ids.none.random.cc1,
