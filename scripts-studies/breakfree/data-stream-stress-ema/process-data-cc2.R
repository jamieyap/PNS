...
load(file.path(path.breakfree.staged_data, "raw.stressEMA.cc2.RData"))

###############################################################################
# Create a question bank file containing info for each EMA item
###############################################################################
df.raw <- list.df.raw.stress.DATA.cc2[[23]]  # Pick one individual
...
if(isTRUE(write.out)){
  write.csv(items.cc2, 
            file.path(path.breakfree.output_data, "stressEMA_items_cc2.csv"), 
            row.names=FALSE, na="") 
}
...
###############################################################################
list.df.raw.stress.STATUS.cc2 <- lapply(list.df.raw.stress.STATUS.cc2, function(x){
...
list.df.raw.stress.STATUS.cc2 <- discard(list.df.raw.stress.STATUS.cc2, is.null)
...
for(i in 1:N){
  ...
  # Save changes
  list.df.raw.stress.DATA.cc2[[i]] <- df.raw
...
source(file.path(path.breakfree.code, "data-stream-stress-ema/grab-single-response-items-cc2.R"))
...
# Grab items with responses find.this.string
source(file.path(path.breakfree.code, "data-stream-stress-ema/grab-multiple-response-items-cc2.R"))
df.resp.cc2 <- df.resp.cc2 %>% mutate_at(vars(grep(pattern = "item.", x = .)), as.character)
...
# DATA files
###############################################################################
list.df.tmp <- lapply(list.df.raw.stress.STATUS.cc2, function(x, use.col.names=colnames(df.collect.all)){
...
df.collect.all <- df.collect.all %>% 
  ...
  group_by(user.id) %>%
  mutate(ones=1) %>%
  mutate(stress.ema.id = cumsum(ones)) %>%
  select(-ones)
...
df.collect.all <- df.collect.all %>%
  select(user.id, stress.ema.id, ema.type, status,
...
if(isTRUE(write.out)){
  write.csv(df.collect.all, 
            file.path(path.breakfree.output_data, "stressEMA_responses_cc2.csv"), 
            row.names=FALSE, na="")
}
