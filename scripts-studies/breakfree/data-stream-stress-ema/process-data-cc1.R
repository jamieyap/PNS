...
load(file.path(path.breakfree.staged_data, "raw.stressEMA.cc1.RData"))

###############################################################################
# Create a question bank file containing info for each EMA item
###############################################################################
df.raw <- list.df.raw.stress.DATA.cc1[[3]]  # Pick one individual
...
if(isTRUE(write.out)){
  write.csv(items.cc1, 
            file.path(path.breakfree.output_data, "stressEMA_items_cc1.csv"), 
            row.names=FALSE, na="")
}
...
for(i in 1:N){
  ...
  df.ref <- df.raw %>%
    mutate(offset.secs = mCerebrum_offset*60) %>%
    select(user.id, merge.id, status, 
           prompt.unixts, begin.unixts, end.unixts, offset.secs,
           with.any.response)
  
  # Save changes
  list.df.raw.stress.DATA.cc1[[i]] <- df.raw
...
source(file.path(path.breakfree.code, "data-stream-stress-ema/grab-single-response-items-cc1.R"))
...
# Grab items with responses find.this.string
source(file.path(path.breakfree.code, "data-stream-stress-ema/grab-multiple-response-items-cc1.R"))
df.resp.cc1 <- df.resp.cc1 %>% mutate_at(vars(grep(pattern = "item.", x = .)), as.character)
...
if(isTRUE(write.out)){
  write.csv(df.collect.all, 
            file.path(path.breakfree.output_data, "stressEMA_responses_cc1.csv"), 
            row.names=FALSE, na="")
}
