...
load(file.path(path.breakfree.staged_data, "raw.hrv.cc2.RData"))
...
datadescriptor.cc2 <- datadescriptor.cc2 %>% 
...
df.raw.hrv.cc2 <- bind_rows(list.df.raw.hrv.cc2)
colnames(df.raw.hrv.cc2)[colnames(df.raw.hrv.cc2) %in% paste("V",3:39,sep="")] <- paste("variable",1:37,sep=".")
colnames(df.raw.hrv.cc2)[colnames(df.raw.hrv.cc2)=="V1"] <- "time.unixts"
df.raw.hrv.cc2 <- df.raw.hrv.cc2 %>% select(-V2)

# Get variable names of HRV-related features
hrv.cols <- datadescriptor.cc2[which(datadescriptor.cc2[["is.hrv.feature"]]==1),"variable.id"]
df.raw.hrv.cc2 <- df.raw.hrv.cc2 %>% select(user.id, time.unixts, hrv.cols)

# Create human-readable timestamp from unix timestamp
df.raw.hrv.cc2 <- df.raw.hrv.cc2 %>%
...
colnames(df.raw.hrv.cc2)[grep("variable.",colnames(df.raw.hrv.cc2))] <- codebook$hrv.variable.id
codebook <- codebook %>% select(-variable.id)

# Write data frames to csv file
write.csv(codebook, file.path(path.breakfree.output_data, "hrv_features_description_cc2.csv"), row.names=FALSE, na="")
write.csv(df.raw.hrv.cc2, file.path(path.breakfree.output_data, "hrv_features_cc2.csv"), row.names=FALSE, na="")
