...
for(i in 1:length(ids.cc2)){
  ...
  idx <- match(x="WAKEUP--org.md2k.studywithema.csv.bz2", table=all.files)
...
# -----------------------------------------------------------------------------
list.df.raw.wakeup.cc2 <- list.df.raw
remove(list.df.raw)

save(list.df.raw.wakeup.cc2, file = file.path(path.breakfree.staged_data, "raw.wakeup.cc2.RData"))
