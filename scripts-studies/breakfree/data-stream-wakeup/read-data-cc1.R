...
for(i in 1:length(ids.cc1)){
  ...
  idx <- grepl(pattern = "WAKEUP+PHONE.csv.zip", x = all.files, fixed=TRUE)
  ...
  if(len>0){
    ...
    idx <- grep("WAKEUP+PHONE.csv.zip", file.here, invert = FALSE)
...
# -----------------------------------------------------------------------------
list.df.raw.wakeup.cc1 <- list.df.raw
remove(list.df.raw)

save(list.df.raw.wakeup.cc1, file = file.path(path.breakfree.staged_data, "raw.wakeup.cc1.RData"))
