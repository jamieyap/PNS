...
for(i in 1:length(ids.cc2)){
  ...
  idx <- match(x="DAY--START--org.md2k.studywithema.csv.bz2", table=all.files)
...
# -----------------------------------------------------------------------------
list.df.raw.pressdaystart.cc2 <- list.df.raw
remove(list.df.raw)

save(list.df.raw.pressdaystart.cc2, file = file.path(path.breakfree.staged_data, "raw.pressdaystart.cc2.RData"))
