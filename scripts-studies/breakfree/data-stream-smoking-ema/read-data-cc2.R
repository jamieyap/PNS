...
for(i in 1:length(ids.cc2)){
  ...
  idx <- match(x="EMA_SMOKING--DATA--org.md2k.ema.csv", table=all.files)
...
list.df.raw.smoking.DATA.cc2 <- list.df.raw
...
for(i in 1:length(ids.cc2)){
  ...
  all.files <- list.files(file.path(path.breakfree.cc2.input_data, this.id))
  # Pick out file names related to data stream of interest
  idx <- match(x="EMA_SMOKING--STATUS--org.md2k.scheduler.csv.bz2", table=all.files)
  # Pick out corresponding files
  this.file <- all.files[idx]
...
list.df.raw.smoking.STATUS.cc2 <- list.df.raw
remove(list.df.raw, all.files)

# -----------------------------------------------------------------------------
# Save objects into an RData file
# -----------------------------------------------------------------------------
save(list.df.raw.smoking.DATA.cc2, 
     list.df.raw.smoking.STATUS.cc2,
     file = file.path(path.breakfree.staged_data, 
                      "raw.smokingEMA.cc2.RData"))
