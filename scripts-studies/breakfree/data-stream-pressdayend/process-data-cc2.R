...
load(file.path(path.breakfree.staged_data, "raw.pressdayend.cc2.RData"))

# Create data frame from list
df.raw.pressdayend.cc2 <- bind_rows(list.df.raw.pressdayend.cc2)

# Sanity check
check.equal <- all_equal(df.raw.pressdayend.cc2$V1,df.raw.pressdayend.cc2$V3)
assert_that(check.equal, msg="Columns V1 and V3 must be equal")

# Clean up columns
df.raw.pressdayend.cc2 <- df.raw.pressdayend.cc2 %>%
