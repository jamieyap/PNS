...
for(i in 1:length(ids.cc2)){
  ...
  idx <- match(x="DAY--END--org.md2k.studywithema.csv.bz2", table=all.files)
...
# -----------------------------------------------------------------------------
list.df.raw.pressdayend.cc2 <- list.df.raw
remove(list.df.raw)

save(list.df.raw.pressdayend.cc2, file = file.path(path.breakfree.staged_data, "raw.pressdayend.cc2.RData"))
