...
for(i in 1:length(ids.cc1)){
  ...
  idx <- grepl(pattern = "DAY_END+PHONE.csv.zip", x = all.files, fixed=TRUE)
...
# -----------------------------------------------------------------------------
list.df.raw.pressdayend.cc1 <- list.df.raw
remove(list.df.raw)

save(list.df.raw.pressdayend.cc1, file = file.path(path.breakfree.staged_data, "raw.pressdayend.cc1.RData"))
